...
async function load() {
  ...
  section.innerHTML = `
    <div class="admin-command-head">
      ...
      <button class="button ghost" data-mark-all>Mark all read</button>
    </div>
    <div class="admin-command-stats">
  ...
  section.querySelector('[data-mark-all]')?.addEventListener('click', async () => { await supabase!.rpc('admin_mark_all_activity_read'); load(); });
  section.querySelectorAll<HTMLElement>('[data-activity]').forEach(el => el.addEventListener('click', async () => { await supabase!.rpc('admin_mark_activity_read', { p_id: el.dataset.activity }); el.classList.add('is-read'); }));
  section.querySelectorAll<HTMLButtonElement>('[data-approve-comment]').forEach(btn => btn.addEventListener('click', async () => { await supabase!.rpc('admin_approve_comment', { p_comment_id: btn.dataset.approveComment }); load(); }));
  section.querySelectorAll<HTMLButtonElement>('[data-reject-comment]').forEach(btn => btn.addEventListener('click', async () => { await supabase!.rpc('admin_reject_comment', { p_comment_id: btn.dataset.rejectComment }); load(); }));
  section.querySelectorAll<HTMLButtonElement>('[data-publish]').forEach(btn => btn.addEventListener('click', async () => { const id=btn.dataset.publish; const input=section.querySelector<HTMLInputElement>(`[data-slug="${id}"]`); await supabase!.rpc('admin_publish_pitch', { p_id:id, p_slug:input?.value ?? '', p_status:'open' }); load(); }));
}
